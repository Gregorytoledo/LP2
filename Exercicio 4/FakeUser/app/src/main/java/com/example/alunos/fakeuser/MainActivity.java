...
public class MainActivity extends AppCompatActivity {
    ...
    private class GetJson extends AsyncTask<Void, Void, FakeUser> {
        ...
        protected void onPostExecute(FakeUser pessoa){
            String srtNome = pessoa.getNome().substring(0, 1).toUpperCase() + pessoa.getNome().substring(1);
            nome.setText(srtNome);

            String srtSobrenome = pessoa.getSobrenome().substring(0, 1).toUpperCase() + pessoa.getSobrenome().substring(1);
            sobrenome.setText(srtSobrenome);

            email.setText(pessoa.getEmail());
            endereco.setText(pessoa.getEndereco());
            String srtCidade = pessoa.getCidade().substring(0, 1).toUpperCase() + pessoa.getCidade().substring(1);
            cidade.setText(srtCidade);
